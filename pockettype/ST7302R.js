...
function init(spi, dc, ce, rst, callback) {
    ...
    cmd(0xC2,[0x00, 0x00, 0x00, 0x00]);      // Source Voltage Control 2
    ...
    cmd(0xB2,0x00,0x05); // frame rate 16Hz high power 4 Hz low power
...
function ST7302(options) {
    function getaddr(buf){
        var a = E.getAddressOf(buf,true);
        if (!a) throw new Error("ST7302 -buffer not a Flat String");
        return a;
    }
    let command = init(options.spi, options.dc, options.ce, options.rst);
    var code = getcode(options.rotated);
    let screenbuf = new Uint8Array(Math.ceil(LCD_HEIGHT/12)*(LCD_WIDTH/2)*3); // cols*rows*3 - 24 pixel chunks
    let g = Graphics.createArrayBuffer(250,128,1,{vertical_byte:true,msb:false});
